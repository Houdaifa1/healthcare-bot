...
export class ReminderService {
  ...
  private async processReminders(): Promise<void> {
    ...
    for (const campaign of runningCampaigns) {
      ...
      if (!clinic) {
        this.logger.warn(
          `Clinic ${campaign.clinicId} not found for campaign ${campaign.id} — skipping`,
        );
        continue;
      }
      ...
      const reminderIntervalHours =
        campaign.reminderIntervalHours ?? clinic.reminderIntervalHours;
      const cutoff = new Date(
        Date.now() - reminderIntervalHours * 60 * 60 * 1000,
      );
      ...
      for (const patient of eligiblePatients) {
        try {
          ...
          this.logger.error(
            `Reminder failed for patient ${patient.id}: ${err.message}`,
          );
        }
      }
    }
  ...
  private async fetchBotMessage(
    ...
    if (!body && language !== Language.FR) {
      this.logger.warn(
        `BotMessage ${key} not found for ${language} — falling back to FR`,
      );
