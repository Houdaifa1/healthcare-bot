...
export class QrController {
  ...
  // ─── SSE — NestJS handles headers, no manual flushHeaders ─────────────────
  @Sse('events')
  streamQrState(@Query('token') token: string): Observable<MessageEvent> {
    this.validateToken(token);
    return interval(1000).pipe(
      map(() => ({ data: JSON.stringify(this.getState()) })),
    );
  }
...
(function () {
  ...
  var stallTimer = setTimeout(function () {
    if (!sseActive) {
      transportBar.textContent = 'transport: polling 2s';
      startPolling();
    }
  }, 6000);
  ...
  function render(data) {
    ...
    if (data.qrDataUrl && data.qrIsValid) {
      ...
      if (data.qrGeneratedAt) qrGenAt = data.qrGeneratedAt;
  ...
  // ── RAF countdown — driven by server qrGeneratedAt timestamp ───────────────
  function startRaf() {
    stopRaf();
    function tick() {
      if (qrGenAt) {
        ...
        var pct       = Math.min(100, Math.max(0, Math.round((remaining / QR_TTL_MS) * 100)));
      ...
      rafH̄andle = requestAnimationFrame(tick);
    }
